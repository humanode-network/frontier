...
use jsonrpsee::{core::traits::IdProvider, PendingSubscription};
	...
	BE::State: StateBackend<BlakeTwo256>,
{
	fn subscribe(
		...
		let starting_block = self.starting_block.clone();
		let fut = async move {
			match kind {
				Kind::Logs => {
					let stream = client
							...
							if let Some(mut sink) = sink.accept() {
				...
				Kind::Syncing => {
						let mut sink = match sink.accept() {
							Some(sink) => sink,
							_ => return,
						};
						...
						let network = Arc::clone(&network);
							...
							let _ = sink
 									.send(&PubSubResult::SyncState(
 										status(client.clone(), network.clone(), starting_block.clone()).await,
 									));
							...
							let last_syncing_status = AtomicBool::new(Arc::clone(&network).is_major_syncing());
							let stream = client.clone().import_notification_stream().filter_map(|_| async {
		...
		self.executor.spawn("frontier-rpc-subscription", Some("rpc"), fut.map(drop).boxed());
	}
}
