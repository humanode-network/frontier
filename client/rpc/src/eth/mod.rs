...
#[async_trait]
impl<B, C, P, CT, BE, H: ExHashT, A> EthApiServer<TxHash<P>> for Eth<B, C, P, CT, BE, H, A>
where
	...
	A: ChainApi<Block = B> + 'static,
{
	...
	fn storage_at(&self, address: H160, index: U256, number: Option<BlockNumber>) -> RpcResult<H256> {
	...
	fn call(&self, request: CallRequest, number: Option<BlockNumber>) -> RpcResult<Bytes> {
...
fn pending_runtime_api<'a, B: BlockT, C, BE, A: ChainApi>(
	client: &'a C,
	graph: &'a Pool<A>,
) -> RpcResult<sp_api::ApiRef<'a, C::Api>>
where
	B: BlockT<Hash = H256> + Send + Sync + 'static,
	C: ProvideRuntimeApi<B> + StorageProvider<B, BE>,
	C: HeaderBackend<B> + Send + Sync + 'static,
